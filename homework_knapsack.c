...
// a) Implement and justify an optimal 0-1 knapsack algorithm in C, with pseudo-polynomial time complexity. Specify the time and space complexity.
// b) Implement a 2-approximation 0-1 knapsack algorithm with O(n) time complexity and O(1) space complexity.
...
// I submitted a similar solution to https://cms.fmi.unibuc.ro/problem/discreteknapsack (with weights being different from values).
static int knapsack_optimal(int max_weight, int len, const int *weights) {
    ...
    // DP[i][w] = max(DP[i - 1][w], DP[i - 1][w - weights[i]] + weights[i]) otherwise (if we choose the item we will add it to the previous optimal solution)
...
// Solution is not reconstructed, though that wouldn't change the time complexity in this case.
// Time complexity is O(n log n) due to sorting. Not sure what could be done about this.
// If the input were already sorted, time complexity would be O(n).
// This mutates the weights input, which is why space complexity is O(1).
static int knapsack_approximation(int max_weight, int len, int *weights) {
    ...
    // Could be made O(n) with radix sort, but that works for fixed-size integers, so not a general solution.
...
void run_test(int *memory, int max_len, int max_max_weight, int max_value) {
    // Compile with -ftrapv -fsanitize=address,undefined for best testing.
    // TODO: don't use rand()
